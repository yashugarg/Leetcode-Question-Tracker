...
function Body({ url }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            await fetch("https://raw.githubusercontent.com/kunal-kushwaha/DSA-Bootcamp-Java/main/" + url)
                .then(res => res.text())
                .then(val => {
    ...
    return (
        <Layout>
            <h1 className={classes.header}>{url.substring(12, url.length - 3).replace("-", " ")}</h1>
            <ul className={classes.categoryController}>
                <li ><button onClick={() => sectionUpdate(0)}
                    style={currentSection === 0 ? activeSectionStyle : {}}>
                    Video
                </button></li>
                <li ><button onClick={() => sectionUpdate(1)}
                    style={currentSection === 1 ? activeSectionStyle : {}}>
                    Easy
                </button></li>
                <li ><button onClick={() => sectionUpdate(2)}
                    style={currentSection === 2 ? activeSectionStyle : {}}>
                    Medium
                </button></li>
                <li ><button onClick={() => sectionUpdate(3)}
